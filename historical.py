...
class CBCHistoryManager:
    def __init__(self, mongodb_uri, db_name="cbc_analyzer", collection_name="patient_records"):
    ...
    def generate_patient_id(self, patient_name=None, patient_age=None, patient_gender=None):
        ...
        if patient_name and patient_age and patient_gender:
            ...
            patient_id = f"CBC-{date_str}-{name_part}{patient_age}{patient_gender[0]}{random_part}"
        else:
            date_str = datetime.now().strftime("%Y%m%d")
            random_part = ''.join(random.choices(string.ascii_uppercase + string.digits, k=4))
            patient_id = f"CBC-{date_str}-{random_part}"
        # Ensure uniqueness
    ...
    def save_patient_record(self, patient_id, patient_info, cbc_results, abnormalities, is_new_patient=False):
...
def format_abnormalities(cbc_results):
...
def create_trend_chart(history_data, parameter):
    ...
    for record in history_data:
        ...
        for result in record.get('cbc_results', []):
...
def patient_history_page(history_manager):
    ...
    st.markdown("Track CBC results over time and analyze patient trends")
    ...
    with col1:
        search_query = st.text_input(
            ...
            placeholder="Enter Patient ID (e.g., CBC-20250702-A1B2) or Patient Name",
...
def show_visit_details(record):
    results_df = pd.DataFrame(record.get('cbc_results', []))
...
def display_patient_history(history_manager, patient_id, use_expander_for_visits=True):
    ...
    with tab4:
        ...
        if len(history_data) >= 2:
            ...
            comparison_data = []
            params1 = {r['Test']: r for r in visit1_data.get('cbc_results', [])}
            params2 = {r['Test']: r for r in visit2_data.get('cbc_results', [])}
