...
from historical_rft import RFTHistoryManager, patient_history_page, format_abnormalities

RFT_REFERENCE_RANGES = {
    "RFT_Reference_Ranges": {
        "version": "2025.1",
        "note": "Reference ranges may vary slightly between laboratories. Always interpret relative to clinical context.",
        "RENAL_MARKERS": {
            "Serum_Creatinine": {
                "male": "0.7-1.3 mg/dL",
        ...
        "ELECTROLYTES": {
            "Sodium (Na+)": {
                "male": "135-146 mmol/L",
                "female": "135-146 mmol/L"
            },
            "Potassium (K+)": {
                "male": "3.5-5.0 mmol/L",
                "female": "3.5-5.0 mmol/L"
            },
            "Chloride (Cl-)": {
                "male": "98-110 mmol/L",
                "female": "98-110 mmol/L"
            },
            "Bicarbonate (CO2)": {
                "male": "22-29 mmol/L",
                "female": "22-29 mmol/L"
        ...
        "OTHER": {
            "Calcium": {
                "male": "8.6-10.2 mg/dL",
                "female": "8.6-10.2 mg/dL"
            },
            "Phosphorus": {
                "male": "2.4-4.7 mg/dL",
                "female": "2.4-4.7 mg/dL"
            }
        },
        "URINE_TESTS": {
            "Random Protein": {
...
def generate_patient_id(patient_name, patient_age, patient_gender):
    ...
    return f"RFT-{date_str}-{name_part}{patient_age}{patient_gender[0]}{random_part}"
...
def get_reference_range(cell_type, gender):
    ranges = RFT_REFERENCE_RANGES["RFT_Reference_Ranges"]
...
def generate_comprehensive_analysis(rft_results, patient_gender, patient_age):
    GROQ_API_KEY = os.environ.get("GROQ_API_KEY")
    try:
        groq_chat = ChatGroq(groq_api_key=GROQ_API_KEY, model_name='llama-3.1-8b-instant')
        prompt = f"""
Analyze RFT results for {patient_age}-year-old {patient_gender.lower()} patient. Provide direct, clinical recommendations without conversational language.

RFT RESULTS:
{json.dumps(rft_results, indent=2)}
...
def rft_analyzer_page():
    st.title("🧪 Renal Function Test (RFT) Analyzer")
    st.markdown("Enter your RF test results for comprehensive medical analysis")
    ...
    with col1:
         st.header("Enter RFT Values")
    st.subheader("🟢 Renal Markers")
    ...
    with renal_col1:
        ...
        bun = st.number_input("Blood Urea Nitrogen (BUN) (mg/dL)", min_value=0.0, step=0.1, format="%.1f")
    with renal_col2:
        uric_acid = st.number_input("Uric Acid (mg/dL)", min_value=0.0, step=0.1, format="%.1f")
        egfr = st.number_input("eGFR (mL/min/1.73m²)", min_value=0.0, step=0.1, format="%.1f")
    
    st.subheader("🔵 Electrolytes")
    electrolyte_col1, electrolyte_col2 = st.columns(2)
    with electrolyte_col1:
        sodium = st.number_input("Sodium (Na+) (mmol/L)", min_value=0.0, step=0.1, format="%.1f")
        potassium = st.number_input("Potassium (K+) (mmol/L)", min_value=0.0, step=0.1, format="%.1f")
    with electrolyte_col2:
        chloride = st.number_input("Chloride (Cl-) (mmol/L)", min_value=0.0, step=0.1, format="%.1f")
        bicarbonate = st.number_input("Bicarbonate (CO2) (mmol/L)", min_value=0.0, step=0.1, format="%.1f")
    
    st.subheader("🟡 Other Parameters")
    other_col1, other_col2 = st.columns(2)
    with other_col1:
        calcium = st.number_input("Calcium (mg/dL)", min_value=0.0, step=0.1, format="%.1f")
        phosphorus = st.number_input("Phosphorus (mg/dL)", min_value=0.0, step=0.1, format="%.1f")
    with other_col2:
        random_protein = st.number_input("Random Protein (mg/dL)", min_value=0.0, step=0.1, format="%.1f")
        microalbumin = st.number_input("Microalbumin (mg/L)", min_value=0.0, step=0.1, format="%.1f")
    with col2:
        st.header("Analysis")
    if st.button("🔍 Analyze RFT & Generate Recommendations", type="primary"):
        results = []
        tests = [
            ("Serum_Creatinine", serum_creatinine, "mg/dL"),
            ("Blood_Urea_Nitrogen (BUN)", bun, "mg/dL"),
            ("Uric_Acid", uric_acid, "mg/dL"),
            ("eGFR (Estimated Glomerular Filtration Rate)", egfr, "mL/min/1.73m²"),
            ("Sodium (Na+)", sodium, "mmol/L"),
            ("Potassium (K+)", potassium, "mmol/L"),
            ("Chloride (Cl-)", chloride, "mmol/L"),
            ("Bicarbonate (CO2)", bicarbonate, "mmol/L"),
            ("Calcium", calcium, "mg/dL"),
            ("Phosphorus", phosphorus, "mg/dL"),
            ("Random Protein", random_protein, "mg/dL"),
            ("Microalbumin", microalbumin, "mg/L")
        ]
        ...
        if results:
                st.session_state.rft_results = results
                ...
                st.info("Please enter at least one RFT value to analyze.")

def ai_assistant_page():
    st.title("🤖 RFT Medical Assistant")
    st.markdown("Comprehensive medical analysis and recommendations based on your RFT results")
    if 'rft_results' in st.session_state and 'ai_analysis' in st.session_state:
        col1, col2 = st.columns([2, 1])
        with col1:
            st.subheader("📊 Your RFT Results Summary")
            df = pd.DataFrame(st.session_state.rft_results)
            abnormal_results = [r for r in st.session_state.rft_results if r["Status"] != "NORMAL"]
            if abnormal_results:
                st.warning(f"⚠️ {len(abnormal_results)} abnormal result(s) found")
                for result in abnormal_results:
                    st.write(f"• **{result['Test']}**: {result['Value']} ({result['Status']})")
            else:
                st.success("✅ All RFT parameters are within normal limits")
        with col2:
        ...
        custom_question = st.text_area("Ask about your RFT results, symptoms, or specific concerns:")
        if st.button("Get Answer") and custom_question:
            try:
                GROQ_API_KEY = os.environ.get("GROQ_API_KEY")
                groq_chat = ChatGroq(groq_api_key=GROQ_API_KEY, model_name='llama-3.1-8b-instant')
                context_prompt = f"""
Based on these RFT results: {st.session_state.rft_results}
...
"""
        ...
        st.info("📝 Please analyze your RFT results first on the main page to get comprehensive recommendations.")
...
def generate_pdf_report(patient_info, rft_results, ai_analysis, doctor_comments, findings, doctor_info=None):
    buffer = io.BytesIO()
    ...
    elements.append(Paragraph("COMPREHENSIVE RFT ANALYSIS REPORT", title_style))
    ...
    rft_data = [['Parameter', 'Result', 'Reference Range', 'Status']]
    for result in rft_results:
        rft_data.append([
            result['Test'],
            result['Value'],
    ...
    rft_table = Table(rft_data, colWidths=[2*inch, 1.5*inch, 2*inch, 1*inch])
    table_style = [
    ...
    for i, result in enumerate(rft_results, 1):
    ...
    rft_table.setStyle(TableStyle(table_style))
    elements.append(rft_table)
    elements.append(Spacer(1, 20))
    if ai_analysis:
...
def pdf_report_page():
    ...
    if 'rft_results' not in st.session_state:
        st.warning("⚠️ Please analyze RFT results first")
        return
    ...
    if st.session_state.get('rft_results'):
        abnormal_count = sum(1 for result in st.session_state.rft_results if result['Status'] != 'NORMAL')
        total_tests = len(st.session_state.rft_results)
        ...
        try:
            ...
            with st.spinner("🔄 Generating comprehensive medical report..."):
                pdf_buffer = generate_pdf_report(
                    ...
                    rft_results=st.session_state.rft_results,
            ...
            # --- Save to MongoDB ---
            if history_manager:
                abnormalities = format_abnormalities(st.session_state.rft_results)
            ...
            filename = f"RFT_Report_{patient_name.replace(' ', '_')}_{test_date.strftime('%Y%m%d')}.pdf"
            st.download_button(
            ...
            st.info(f"📊 Report includes {len(st.session_state.rft_results)} RFT parameters with comprehensive analysis")
        except Exception as e:
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("🔄 Update RFT Results"):
            st.info("Please return to the main analyzer page to update your RFT results")
    with col2:
    ...
    **Your PDF report will include:**
    • Complete RFT results with reference ranges
...
def main():
    ...
    if 'history_manager' not in st.session_state:
        st.session_state.history_manager = RFTHistoryManager(MONGODB_URI)
    ...
    if 'rft_results' in st.session_state:
        total_tests = len(st.session_state.rft_results)
        abnormal_count = sum(1 for r in st.session_state.rft_results if r.get('Status', '') != 'NORMAL')
        ...
        st.sidebar.info("No RFT results yet")
    st.sidebar.markdown("---")
...
def about_page():
    st.title("ℹ️ About RFT Analyzer Pro")
    st.markdown("""
    <div class="main-header">
        <h1>RFT Analyzer Pro - Advanced Renal Test Analysis</h1>
    </div>
    """, unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        ...
        st.markdown("""
        • **Comprehensive RFT Analysis** - All major parameters
        ...
        """)
    with col2:
        st.subheader("🧪 RFT Parameters Analyzed")
        st.markdown("""
         Uric Acid & eGFR
         Electrolytes: Sodium, Potassium, Chloride, Bicarbonate
         Urine Tests: Random Protein, Microalbumin        
        """)
    ...
    with version_col1:
        ...
        **Last Updated:** RFT Reference Ranges 2025.1
        """)
    with version_col2:
        st.markdown("""
        **Recent Updates:**
